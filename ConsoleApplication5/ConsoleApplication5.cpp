...
void playGame(char** playerBoard, char** computerBoard, int shipSizes[], int LENGTH_SIZE, int WIDTH_SIZE, Ships* Player_Ships, Ships* Computer_Ships) {
    ...
    while (Cnt_Computer_Ships != 0 && Cnt_Player_Ships != 0) {
        ...
        cout << "Computer's ships " << Cnt_Computer_Ships << endl;
        ...
        if (Cnt_Computer_Ships == 0) {
            ...
            cout << "Player's ships " << Cnt_Player_Ships << endl;
            cout << "Press Enter to continue..." << endl;
            cin.ignore();
            getchar();
        }
...
int main() {
    ...
    if (answer == "CU") {
        cout << "Please, write sizes of ships" << endl;
        i = 0;
        while (i < NUM_SHIPS) {
            cin >> shipSizes[i];
            if (cin.fail()) {
            ...
            if (shipSizes[i] > max(LENGTH_SIZE, WIDTH_SIZE)) {
                ...
                if (sum >= (LENGTH_SIZE * WIDTH_SIZE) / 2) {
                    cout << "Invalid input. Too many ships, please write again" << endl;
                    i = 0;
                    continue;
                }
            }
    ...
    playGame(playerBoard, computerBoard, shipSizes, LENGTH_SIZE, WIDTH_SIZE, Player_Ships, Computer_Ships);
    return 0;
}
